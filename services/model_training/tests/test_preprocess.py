...
class TestPreprocess(unittest.TestCase):
    ...
    def test_select_and_transform_features(self):
        ...
        self.assertCountEqual(df_context.columns, config.CONTEXT_COLS_CAT + config.CONTEXT_COLS_NUM)
    ...
    def test_run_preprocessing_integration(self):
        ...
        X_final, famd_model, scaler = preprocess.run_preprocessing(df_context, df_behavioral)
    ...
    def test_run_preprocessing_mocked_logic(self, MockScaler, MockFAMD):
        ...
        mock_famd_instance.fit_transform.return_value = pd.DataFrame(np.random.rand(6, 3))
        MockFAMD.return_value = mock_famd_instance
        
        mock_scaler_instance = MagicMock()
        mock_scaler_instance.fit_transform.return_value = np.random.rand(6, len(config.BEHAVIORAL_COLS))
        MockScaler.return_value = mock_scaler_instance

        df_context, df_behavioral = preprocess.select_and_transform_features(self.df)
        X_final, _, _ = preprocess.run_preprocessing(df_context, df_behavioral)

        MockFAMD.assert_called_once_with(n_components=config.FAMD_COMPONENTS, random_state=42)
