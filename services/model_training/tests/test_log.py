...
class TestLog(unittest.TestCase):
    ...
    def test_log_experiment(self, mock_silhouette, mock_dbcv, mock_log_artifacts, mock_log_metrics, mock_log_metric, mock_log_params, mock_log_param, mock_joblib_dump):
        ...
        # Assert
        mock_log_param.assert_called_once_with("famd_components", config.FAMD_COMPONENTS)
        mock_log_params.assert_called_once_with(config.HDBSCAN_PARAMS)
        mock_log_metrics.assert_called_once_with({"initial_noise_ratio": 0.2, "dbcv_score": 0.8})
        mock_log_metric.assert_called_once_with("final_silhouette_score", 0.7)
        self.assertEqual(mock_joblib_dump.call_count, 4)
        mock_log_artifacts.assert_called_once_with(config.ARTIFACT_PATH, artifact_path="results")