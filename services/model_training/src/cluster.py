...
def run_clustering(X_scaled: pd.DataFrame, original_df: pd.DataFrame):
    ...
    noise_ratio = (
        (df_clustered["cluster"] == -1).sum() / len(df_clustered)
        if len(df_clustered) > 0
        else 0
    )
    logging.info(
        f"HDBSCAN found {df_clustered['cluster'].nunique()} clusters with {noise_ratio:.2%} noise."
    )
    ...
    if noise_mask.sum() > 0:
        ...
        noise_classifier.fit(
            X_scaled[clean_mask], df_clustered.loc[clean_mask, "cluster"]
        )
