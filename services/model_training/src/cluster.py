...
def run_clustering(X_scaled: pd.DataFrame, original_df: pd.DataFrame):
    ...
    """
    print("--- Running Clustering (HDBSCAN) ---")
    ...
    print(f"HDBSCAN found {df_clustered['cluster'].nunique()} clusters with {noise_ratio:.2%} noise.")
    ...
    if noise_mask.sum() > 0:
        print(f"Classifying {noise_mask.sum()} noise points...")
