...
def main():
    ...
    st.markdown(
        "An end-to-end platform for identifying store archetypes and deriving actionable business insights."
    )
    ...
    if df_clustered is None:
        st.error(
            "Clustered data not found. Please run the full pipeline via Dagster to generate model artifacts."
        )
        st.info(
            "To run the pipeline: `make run-platform`, then trigger the materializations in the Dagster UI at http://localhost:3000."
        )
        return
    ...
    inferred_percentage = (
        (inferred_stores / total_stores) * 100 if total_stores > 0 else 0
    )
    ...
    kpi3.metric(
        ...
        f"{inferred_percentage:.1f}%",
        help="Percentage of stores initially classified as noise by HDBSCAN and subsequently assigned to the nearest cluster.",
    )

    # --- Tab Definitions ---
    tab_viz, tab_actions, tab_integrity = st.tabs(
        ["Cluster Explorer", "Strategic Insights", "Data Integrity"]
    )
