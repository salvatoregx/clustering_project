...
def show(df):
    ...
    if not available_cols:
        st.error(
            "Required features for visualization are not available in the dataset."
        )
        return
    ...
    selected_cluster = st.selectbox(
        "Select a cluster to analyze:", clusters, format_func=lambda x: f"Cluster {x}"
    )
    ...
    if selected_cluster is not None:
        ...
        with col1:
            ...
            df_radar_norm = pd.DataFrame(
                scaler.fit_transform(df[available_cols]), columns=available_cols
            )
            df_radar_norm["cluster_final"] = df["cluster_final"].values

            cluster_means_norm = df_radar_norm[
                df_radar_norm["cluster_final"] == selected_cluster
            ][available_cols].mean()
