...
def generate_cluster_persona(cluster_profile, overall_profile):
    ...
    if cluster_profile["revenue_per_sqm"] > overall_profile["revenue_per_sqm"] * 1.2:
        ...
        recommendations.append(
            "Capitalize on high foot traffic; consider premium product placement."
        )
    elif cluster_profile["revenue_per_sqm"] < overall_profile["revenue_per_sqm"] * 0.8:
        persona.append("Low Efficiency")
        recommendations.append(
            "Review store layout and staff performance; potential for operational improvements."
        )
    ...
    if cluster_profile["size_m2"] > overall_profile["size_m2"] * 1.2:
        ...
        recommendations.append(
            "Utilize space for in-store events or expanded product lines."
        )
    ...
    if cluster_profile["avg_ticket_item"] > overall_profile["avg_ticket_item"] * 1.1:
        ...
        recommendations.append(
            "Target with loyalty programs and premium service offerings."
        )
    else:
        persona.append("Volume-Driven")
        recommendations.append(
            "Focus on promotions, bundle deals, and high-volume items."
        )

    if not persona:
        return "Standard Performers", [
...
def show(df):
    ...
    st.markdown(
        "Each cluster is analyzed against the fleet average to generate a persona and suggest targeted business actions."
    )
    ...
    for cluster_id, profile_row in cluster_profiles.iterrows():
        ...
        persona, recommendations = generate_cluster_persona(
            profile_row, overall_profile
        )
